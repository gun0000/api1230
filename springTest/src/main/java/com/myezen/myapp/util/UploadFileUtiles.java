...
public class UploadFileUtiles {
	...
	throws Exception{
		...
		UUID uid = UUID.randomUUID();//랜덤번호뽑아내는
...
//		System.out.println("realpath:"+realpath);
		...
		String savedPath = calcPath(uploadPath);//경로를만든다
		
		File target = new File(uploadPath+savedPath,savedName);//파일을만들어준다
//  	등록한 파일 상대경로
//		String loc = target.getCanonicalPath();

		FileCopyUtils.copy(fileData,target);//파일을복사를해서
		
		String formatName = originalName.substring(originalName.lastIndexOf(".")+1);//파일이 무슨파일인가 본다 .확장자추출
		System.out.println("formatName:"+formatName);
		String uploadedFileName = null;
		
		if(MediaUtils.getMediaType(formatName) != null){//이 파일형태 맞으면 썸네일을 만들어라
	...
	private static String calcPath(String uploadPath){	
		...
		makeDir(uploadPath, yearPath, monthPath, datePath);//디렉토리를 만들어준다
